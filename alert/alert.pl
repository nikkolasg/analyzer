...
# version 0.9.6  2015-01-29 ngailly modify only for sending sms  & email
...
sub write_log {
  ...
  $return = 0;
  if ($level >= $level_email ) {
    $msg .= "\n\nProcessing has NOT been stopped\nbut you might want to check during the day ;-)\n";
    $return += &send_email(\@EMAIL_ALARM,$msg,$loglevels{$level})  ;
    if ($return != 0) {
        print STDERR "Error while sending mail ...\n"
    }
  }
  if ($level >= $level_sms)  {
    # No processing has not been stopped etc because sms must be small ideally
    $return += &send_sms(\@SMS_ALARM,$msg,$loglevels{$level});
    if ($return != 0) {
        print STDERR "Error while sending sms ...\n"
    }
  }
  exit $return;
}
...
sub send_sms {
  ...
  $subject= 'Analyzer Report - ' . $subject;
  #$subject = $SCRIPTNAME.' - '.$subject;

  $sms_bin = File::Spec->join($EXECUTIONDIR,"send_sms_kannel.pl");
