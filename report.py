...
class Report:
    ...
    def report_anomalies(self,anomalies):
        ...
        if not args.noalert:
            ## We can go on""
            ## for mail
            cmd = 'echo "{}" | ./alert/mail.py "{}" '.format(msg,Config.mail)
            cmd += ' --subject "{}" '.format("Report - ANOMALY")
            cmd += ' --footer "{}" '.format(Config.footer)
            if self.fname is not None: cmd += ' --image "{}" '.format(self.fname)
            log.debug("Will send email with : {}".format(cmd))
            os.system(cmd)

            ## for sms now
            msg += "\n" + Config.footer
            cmd = 'echo "{}" | ./alert/send_sms_kannel.pl {}'.format(msg," ".join(Config.sms_numbers))
            log.debug("Will send sms with : {}".format(cmd))
            os.system(cmd)
    ...
    def generate_plot(self,subplot,name,graph):
        """ Generate a plot on t§he subplot object, in the row-th row,
